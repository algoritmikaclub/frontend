...
function flipImage(image, flipType='h') {
    ...
    if(flipType === 'h') {
        ...
        FIContext.drawImage( image, -flippedImage.width, 0)
    } else if (flipType === 'v') {
        // scaleX, skewY, skewX, scaleY,  offsetX, offsetY
        FIContext.setTransform(1, 0, 0, -1, 0, 0)
...
function rotateImage(image, angle = 180) {
    ...
    // смещаем контекст к центру изображения
    // scaleX, skewY, skewX, scaleY,  offsetX, offsetY
...
function draw() {
    ...
    context.drawImage(flipImage(images['snowman_3.png'], 'h'), 420, 420, 72, 109)
    ...
    for(let i = 0; i < 150; i++) {
        const imageName = `snowflake_${i % 5 + 1}.png`
        const snowflake = rotateImage( images[imageName], Math.random() * 360 )
        ...
        const snowflakeSize = (0.3 + Math.random() * 0.7) * snowflake.width
        context.drawImage(snowflake, snowflakeX, snowflakeY, snowflakeSize, snowflakeSize)
    }
}